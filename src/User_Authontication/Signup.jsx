...
const Signup = () => {
  ...
  const onSubmit = (data) => {
    ...
    signup(email, password)
      ...
      .catch((error) => {
        if (error) {
          toast.warning("Signup process failed");
        }
      });
  };
  ...
  return (
    <div>
      <Helmet>
        <title> Family Home || sign up</title>
      </Helmet>
      <div
        data-aos="zoom-in"
        className=" w-full max-w-md m-auto  p-8 space-y-3 rounded-xl bg-gray-900 dark:bg-gray-50 text-gray-100 dark:text-gray-800"
      >
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div className="space-y-1 text-sm">
            ...
            />
            {/* <small className="text-danger text-red-500 ">{errors.name && errors.name.message}</small> */}
            ...
            <input
              ...
              {...register("password", {
                ...
                pattern: {
                  value: /(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{6,}/,
                  message:
                    "Must have an uppercase letter, a lowercase letter, and be at least 6 characters long",
                },
              })}
              className="w-full px-4 py-3 rounded-md border-gray-700 dark:border-gray-300 bg-gray-900 dark:bg-gray-50 text-gray-100 dark:text-gray-800 focus:border-my_color-400 focus:dark:border-my_color-600"
            />
